...
class CRnnLearner(object):
    def __init__(self,
            ...
            init_lr=0.002, # initial learning rate
    ...
    # index_filter: a 1D numpy array containing 0 and 1 for selecting rows
    def fit(self, X, Y, index_filter=None):
        ...
        self.log("--------------------------------------------------------------")
        m_train = computeMetric(T_tr, P_tr, self.is_regr)
        for m in m_train:
            self.log("Metric: " + m)
            self.log(m_train[m])
        if self.test is not None:
            self.log("--------------------------------------------------------------")
            m_test = computeMetric(T_te, P_te, self.is_regr)
            for m in m_test:
                self.log("Metric: " + m)
                self.log(m_test[m])
        self.log("--------------------------------------------------------------")
...
class ResNet(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        c = nn.Sequential(
            ...
            nn.Conv2d(channel_size, out_size_c1, kernel_size=(3, 1), padding=0, stride=(1, 1), bias=False))
        out_size_r1 = 64
        r1 = ResLargeBlock(out_size_c1, output_size, h_size=16, s_size=1)
        #out_size_r2 = 128
        #r2 = ResLargeBlock(out_size_r1, out_size_r2, h_size=32, s_size=2)
        #out_size_r3 = 256
        #r3 = ResLargeBlock(out_size_r2, out_size_r3, h_size=64, s_size=2)
        #out_size_r4 = 512
        #r4 = ResLargeBlock(out_size_r3, out_size_r4, h_size=128, s_size=2)
...
class CRNN(nn.Module):
    def __init__(self, channel_size, height, width, output_size, use_rnn=False, use_cuda=True):
        ...
        if use_rnn:
            ...
            self.rnn = ResRnnBlock(rnn_input_size, rnn_input_seq_len)
